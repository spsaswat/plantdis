...
void main() {
  group('GeminiService', () {
    ...
    group('getAnswer Method Tests', () {
      ...
      test(
        'getAnswer returns string for non-plant related question',
        () async {
          final result = await geminiService.getAnswer(
      ...
      test(
        'getAnswer handles empty question gracefully',
        () async {
          final result = await geminiService.getAnswer(
            "",
            isPlantRelated: true,
      ...
      test(
        'getAnswer response format is clean for plant questions',
        () async {
          final result = await geminiService.getAnswer(
          ...
          expect(
            result.contains('*'),
            false,
          ); // No asterisk emphasis (except in bullet points)
      ...
      test(
        'getAnswer response format is clean for general questions',
        () async {
          final result = await geminiService.getAnswer(
    ...
    group('Error Handling Tests', () {
      test(
        'getAnswer handles special characters in question',
        () async {
          final result = await geminiService.getAnswer(
      ...
      test(
        ...
        () async {
          final longQuestion =
              "What is the best way to treat " * 50 + "plant diseases?";
    ...
    group('Response Quality Tests', () {
      test(
        'plant related responses should be concise',
        () async {
          final result = await geminiService.getAnswer(
      ...
      test(
        'responses should not contain excessive newlines',
        () async {
          final result = await geminiService.getAnswer(
      ...
      test(
        'responses should be trimmed properly',
        () async {
          final result = await geminiService.getAnswer(
            "Plant care tips",
            isPlantRelated: true,
